...
func (adapter *ArrayAdapter) Set(variable *com.Variable, params map[string]interface{}, expression _interface.ExprCalculate, variable2 _interface.Variable) {
...
func (adapter *ArrayAdapter) GetValue() (interface{}, error) {
	//解析obj
	if eUtils.IsNil(adapter.variable.Array) {
		return nil, errors.New(fmt.Sprintf("%s arrayAdapter array is nil", adapter.variable.EName))
	}
	arrV := make([]com.CalculateObj, len(adapter.variable.Array))
	copy(arrV, adapter.variable.Array)
	EName := adapter.variable.EName
	arrType := adapter.variable.ArrayType
	...
	case conf.ReturnInt:
		...
		for k, val := range arrV {
			...
			err := adapter.variableParse.Set(kName, adapter.params, false, &val)
			if err != nil {
				return nil, err
			}
	...
	case conf.ReturnArray:
		...
		return rt, nil
	default:
		return nil, errors.New(fmt.Sprintf("%s arrayAdapter 不合法的数据类型", EName))
	}
}
